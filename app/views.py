...
class ProductcrudVS(ViewSet):
    ...
    def create(self,request):
        SD=ProductSerializer(data=request.data) #we are updating data from browser
        if SD.is_valid():
            SD.save()
    ...
    def update(self,request,pk):
        SPO=Product.objects.get(pk=pk)
        SPD=ProductSerializer(SPO,data=request.data) #we are updating data from browser
        if SPD.is_valid():
            SPD.save()
    ...
    def partial_update(self,request,pk):
        ...
        if SPD.is_valid():
            SPD.save()
            return Response({'success':'product is parcially updated'})
        else:
            return Response({'faild':'product is not updated partially'})
    def delete(self,request,pk):
        Product.objects.get(pk=pk).delete()
        return Response({'success':'product is deleted successfully'})
